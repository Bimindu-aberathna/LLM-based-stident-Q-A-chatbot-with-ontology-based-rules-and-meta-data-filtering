"""
Metadata filtering manager - clean version for easy import
Contains the main apply_rule_based_filters function that can be imported anywhere
"""

from typing import Dict, List, Tuple
from app.models.chat import StudentQueryRequest

# Direct imports to avoid import issues
import sys
import os
...
# Try to import hierarchical ranking
try:
    from Ontology_ranking import apply_hierarchical_ranking
except ImportError:
    print("Warning: apply_hierarchical_ranking not found, using fallback")
    def apply_hierarchical_ranking(chunks, student):
        # Fallback: just return chunks sorted by similarity
...
def apply_rule_based_filters(
    ...
    Args:
        results: Dictionary with 'documents', 'metadatas', 'distances' from ChromaDB query
        studentMetadata: Student information for filtering
        collection: ChromaDB collection instance for neighbor retrieval
    
    Returns:
        Tuple of (academic_chunks_list, non_academic_chunks_list)
    """
    ...
    neighbor_retriever = ChunkNeighborRetriever(collection) if collection else None
    ...
    for chunk in academic_chunks:
        ...
        if neighbor_retriever:
            ...
            # No neighbor retrieval available, just add original
            final_academic_list.append(chunk['text'])
    ...
    for chunk in ranked_non_academic:
        ...
        print(f"Chunk score: {chunk.get('total_score', 0)}")
        
        if neighbor_retriever:
            chunk_with_neighbors = neighbor_retriever.retrieve_neighbor_chunks_for_a_chunk(
                chunk=chunk['text'],
                chunk_type="non-academic",
                chunk_score=chunk.get('total_score', 0),
                neighbor_count=2  # Only 2 neighbors on each side
            )
            ...
            if chunk_with_neighbors:
                ...
                for neighbor_chunk in chunk_with_neighbors:
                    if neighbor_chunk['is_original_chunk']:
                        annotated_chunk = f"{neighbor_chunk['text']} [ORIGINAL-SCORE: {chunk.get('total_score', 0):.1f}]"
                    else:
                        annotated_chunk = f"{neighbor_chunk['text']} [NEIGHBOR-SCORE: {chunk.get('total_score', 0)*0.9:.1f}]"
                    final_non_academic_list.append(annotated_chunk)
            else:
                print(f"No neighbors found for non-academic chunk, adding original")
                annotated_chunk = f"{chunk['text']} [SCORE: {chunk.get('total_score', 0):.1f}]"
                final_non_academic_list.append(annotated_chunk)
        else:
            # No neighbor retrieval available, just add original with score
            annotated_chunk = f"{chunk['text']} [SCORE: {chunk.get('total_score', 0):.1f}]"
            final_non_academic_list.append(annotated_chunk)
