...
1. "success" - If query is about ANY university-related topic including:
   ...
   - Timetables, schedules, deadlines, Exam dates
  ...
  "rewritten_query": "detailed rewritten query for document search"
    ...
    try:
        ...
        # Handle success status
            ...
            print("re-written query: ", rewritten_query)
            try:
                ...
                if academic_chunks or non_academic_chunks:
                    ...
                    documents, source_lst = "", ""
                    if ext_docs:
                        documents = "\n---CHUNK_SEPARATOR---\n".join(ext_docs)
                    if ext_doc_names:
                        source_lst = " | ".join(ext_doc_names)
                    try:
                    ...
                    try:
                        lmsServiceManager = LLMServiceManager()
                        llm_response = lmsServiceManager.no_relevant_documents_response(
                            question=request.message,
                            studentMetadata=request
                        )
                        if llm_response.get("success", False):
                            answer_message = llm_response.get("answer", answer_message)
                    except Exception as llm_error:
                        print(f"LLM no document response error: {llm_error}")
                    ...
                    try:
                        ...
                        data_logger.log_evaluation_data(
                            eval_type="enhanced",
                            llm_response=llm_response,
                            document_names=[],  # Empty list since no documents found
                            data_chunks=[],     # Empty list since no chunks retrieved
                            message=request.message,
                            q_id=request.Q_id,
                            req_body=request
                        )
...
For "success" cases, rewrite the query to be more specific and searchable.
        ...
        # Handle success status
            ...
            try:
                ...
                if data_chunks:
                    ...
                    documents, source_lst = "", ""
                    if document_names:
                        source_lst = " | ".join(document_names)
                    if data_chunks:
                        documents = "\n---CHUNK_SEPARATOR---\n".join(data_chunks)
